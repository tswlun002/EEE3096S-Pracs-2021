...
void initGPIO(void){
        ...
        //Set up the Buttons
        for(int j=0; j < sizeof(BTNS)/sizeof(BTNS[0]); j++){
                pinMode(BTNS[j], INPUT);    //set as inputmode
                pullUpDnControl(BTNS[j], PUD_UP);  //pullup for buttons
        }

        //Attach interrupts to Buttons
        wiringPiISR (0, INT_EDGE_RISING ,&hourInc); //button for hours control
        wiringPiISR (2, INT_EDGE_RISING ,&minInc);  // button for minutes control
...
int main(void){
        ...
        wiringPiI2CWriteReg8(RTC, MIN_REGISTER, 0x04);
